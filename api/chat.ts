...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { userMessage, dogName, dogAge, chatHistory, ragContext } = req.body as {
      userMessage: string;
      dogName?: string;
      dogAge?: string;
    ...
    const hasContext = ragContext && ragContext.length > 0;
...
이 강아지의 나이에 맞는 조언을 제공해주세요.`;
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: `API 오류: ${JSON.stringify(errorData)}` });
    }
