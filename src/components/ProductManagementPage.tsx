      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label htmlFor="stock" className="block text-sm font-semibold text-neutral-700 mb-2">
                재고 수량 *
              </label>
              <input
                type="number"
                ...
                <div className="space-y-3">
                  {options.map((option, index) => (
                    <div key={index} className="border border-neutral-300 rounded-lg p-4 bg-neutral-50">
                      ...
                      <div className="space-y-3">
                        <div>
                          <label htmlFor={`option-name-${index}`} className="block text-xs font-semibold text-neutral-600 mb-1">
                            옵션명 (예: 사이즈, 색상)
                          </label>
                          <input
                            type="text"
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-neutral-800">물품 재고 관리</h2>
        <button
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <table className="w-full">
            <thead className="bg-neutral-100">
              <tr>
                <th className="px-6 py-3 text-left text-sm font-semibold text-neutral-700">카테고리</th>
                <th className="px-6 py-3 text-left text-sm font-semibold text-neutral-700">물품명</th>
                <th className="px-6 py-3 text-center text-sm font-semibold text-neutral-700">재고</th>
                <th className="px-6 py-3 text-center text-sm font-semibold text-neutral-700">관리</th>
              </tr>
            </thead>
            <tbody>
              {filteredProducts.map((product, index) => (
                <tr key={product.id} className={index % 2 === 0 ? 'bg-white' : 'bg-neutral-50'}>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-sm font-semibold ${getCategoryBadge(product.category)}`}>
                      {product.category}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-semibold text-neutral-800">{product.name}</td>
                  ...
                  <td className="px-6 py-4 text-center">
                    <button
                      ...
                      className="bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-lg mr-2 transition-colors"
                    >
                      수정
                    </button>
                    <button
